...
export async function notifyTeam(lead: LeadForEmail): Promise<void> {
  const sender = {
    email: process.env.BREVO_SENDER_EMAIL || "noreply@klemortgage.com",
    name: process.env.BREVO_SENDER_NAME || "KLE Mortgage Site",
  };
  const to = (process.env.LEAD_NOTIFY_EMAIL || "Info@klemortgage.com")
    .split(",")
    .map((e) => ({ email: e.trim() }))
    .filter((r) => r.email);
  ...
  const adminUrl = `${process.env.NEXT_PUBLIC_SITE_URL || "https://klemortgage.com"}/admin/leads/${lead.id}`;

  const htmlContent = `
    <div style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;color:#0D0608">
      <div style="border-left:3px solid #C9A345;padding-left:16px;margin-bottom:24px">
        <div style="color:#C9A345;font-size:11px;letter-spacing:0.18em;text-transform:uppercase;font-weight:700">New Lead — KLE Mortgage</div>
        <h1 style="margin:6px 0 0 0;font-size:22px;font-weight:700">${escape(fullName)}</h1>
      </div>
      <table style="width:100%;border-collapse:collapse;font-size:14px">
        <tr><td style="padding:8px 0;color:#6b7280;width:130px">Email</td><td style="padding:8px 0"><a href="mailto:${escape(lead.email)}" style="color:#6B1C23">${escape(lead.email)}</a></td></tr>
        <tr><td style="padding:8px 0;color:#6b7280">Phone</td><td style="padding:8px 0"><a href="tel:${escape(lead.phone)}" style="color:#6B1C23">${escape(lead.phone || "—")}</a></td></tr>
        ...
        <tr><td style="padding:8px 0;color:#6b7280">Assigned</td><td style="padding:8px 0">${escape(lead.assignedOfficerName || "Unassigned")}</td></tr>
      </table>
      ${
        lead.message
          ? `<div style="margin-top:20px;padding:16px;background:#F8F6F3;border-radius:10px;font-size:14px;line-height:1.6;white-space:pre-wrap">${escape(lead.message)}</div>`
          : ""
      }
      <div style="margin-top:28px">
        <a href="${escape(adminUrl)}" style="display:inline-block;background:#6B1C23;color:#fff;padding:12px 22px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px">View in Admin →</a>
      </div>
...
const AUTORESPONDER_COPY: Record<Locale, { subject: string; greeting: (n: string) => string; body: string; signoff: string }> = {
  en: {
    subject: "We received your inquiry — KLE Mortgage Financing",
    greeting: (n) => `Hi ${n},`,
    body: "Thanks for reaching out to KLE Mortgage Financing, LLC. A licensed loan officer will get back to you within one business hour. If your need is urgent, call us directly at (305) 705-2030.",
    signoff: "— The KLE Mortgage Team",
  },
  fr: {
    subject: "Nous avons bien reçu votre demande — KLE Mortgage",
    greeting: (n) => `Bonjour ${n},`,
    body: "Merci d'avoir contacté KLE Mortgage Financing, LLC. Un agent de prêt licencié vous répondra dans l'heure suivante (jours ouvrables). Pour toute urgence, appelez-nous directement au (305) 705-2030.",
    signoff: "— L'équipe KLE Mortgage",
  },
  ht: {
    subject: "Nou resevwa demann ou — KLE Mortgage",
    greeting: (n) => `Bonjou ${n},`,
...
export async function sendAutoresponder(lead: LeadForEmail, locale: Locale = "en"): Promise<void> {
  ...
  const sender = {
    email: process.env.BREVO_SENDER_EMAIL || "noreply@klemortgage.com",
    name: process.env.BREVO_SENDER_NAME || "KLE Mortgage Financing",
  };

  const htmlContent = `
    <div style="font-family:-apple-system,Segoe UI,Helvetica,Arial,sans-serif;max-width:560px;margin:0 auto;padding:28px;color:#0D0608;line-height:1.6">
      <div style="border-bottom:1px solid #e5e7eb;padding-bottom:18px;margin-bottom:22px">
        <div style="color:#C9A345;font-size:11px;letter-spacing:0.18em;text-transform:uppercase;font-weight:700">KLE Mortgage Financing, LLC</div>
        <div style="color:#9ca3af;font-size:12px;margin-top:4px">North Miami Beach, FL · NMLS #2380070</div>
      </div>
      ...
      <div style="margin-top:24px;padding-top:18px;border-top:1px solid #e5e7eb;font-size:12px;color:#9ca3af">
        909 N Miami Beach Blvd, Suite 301A · North Miami Beach, FL 33162 · (305) 705-2030 · NMLS #2380070
      </div>
    </div>
  `.trim();
