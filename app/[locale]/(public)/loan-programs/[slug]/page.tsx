...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // For the 7 main slugs we have translated meta in the catalog;
  // for any extras, fall back to the program's English name + description.
  const t = await getTranslations({ locale, namespace: "LoanProgramsMeta" });
  let title: string;
  ...
  try {
    ...
    title = `${program.name} | KLE Mortgage`;
